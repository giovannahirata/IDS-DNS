# versão do svm.py usando o shap
...
benign_dfs = [pd.read_csv(f, nrows=2000) for f in benign_files]
...
malware_df = pd.read_csv('output-of-malware-pcap.csv', nrows=2000)
malware_df['maligno'] = 1
malware_df['tipo_maligno'] = 'Malware'

phishing_df = pd.read_csv('output-of-phishing-pcap.csv', nrows=2000)
phishing_df['maligno'] = 1
phishing_df['tipo_maligno'] = 'Phishing'

spam_df = pd.read_csv('output-of-spam-pcap.csv', nrows=2000)
spam_df['maligno'] = 1
...
modelo = svm_bin.fit(X_train_bin_scaled, y_train_bin)
...
# Shap para explicar modelo
background = shap.sample(X_train_bin_scaled, 200, random_state=42)
explainer = shap.KernelExplainer(modelo.predict_proba, background)
shap_values = explainer.shap_values(X_test_bin_scaled[:50])
shap.summary_plot(shap_values, X_test_bin_scaled, feature_names=selected_columns)


# 5. Classificação Multiclasse (tipos malignos) ------------------------------
...
# Shap para explicar modelo
background = shap.sample(X_train_multi_scaled, 200, random_state=42)
explainer = shap.KernelExplainer(modelo.predict_proba, background)
shap_values = explainer.shap_values(X_test_multi_scaled[:50])
shap.summary_plot(shap_values, X_test_multi_scaled, feature_names=selected_columns)
